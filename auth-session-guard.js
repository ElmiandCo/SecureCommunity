/* OneMuslim — dynamic cinematic secure-session loader.
   ...
   crescent, horizon, mosque silhouettes, parallax and living light.
...
(function(){
  ...
  style.textContent=`
    ...
    #authBootGuard .om-loader-scene{position:absolute;inset:0;overflow:hidden;background:radial-gradient(circle at 50% 58%,#173d35 0,#0b2924 38%,#04120f 100%)}
  ...
  function resize(){
    ...
    for(let i=0;i<150;i++) stars.push({x:rand(0,W),y:rand(0,H*.72),r:rand(.35,1.35),a:rand(.25,.95),phase:rand(0,Math.PI*2)});
    for(let i=0;i<30;i++) particles.push({x:rand(0,W),y:rand(H*.15,H*.75),r:rand(.7,1.7),speed:rand(.08,.28),phase:rand(0,Math.PI*2)});
  }
  ...
  function draw(time){
    ...
    // Milky Way haze
    ctx.save(); ctx.translate(px*-5,py*-3); ctx.rotate(-.48);
    ...
    for(const st of stars){
      ...
      ctx.globalAlpha=st.a*tw; ctx.fillStyle='#fff0bf'; ctx.beginPath(); ctx.arc(st.x+px*-3,st.y+py*-2,st.r,0,Math.PI*2); ctx.fill();
    }
    ctx.globalAlpha=1;

    // Crescent
    const moonX=W*.17+px*-4, moonY=H*.47+py*-3, mr=Math.min(W,H)*.035;
    ...
    // Seven golden orbital arcs
    const cx=W/2+px*-2, cy=H*.37+py*-2, base=Math.min(W*.43,H*.23);
    ...
    // Central emblem
    const ey=H*.47+py*-3;
    ctx.save(); ctx.translate(W/2+px*-2,ey);
    ...
    // Horizon glow
    const hy=H*.69+py*1; const hg=ctx.createRadialGradient(W/2,hy,0,W/2,hy,W*.48); hg.addColorStop(0,'rgba(244,190,91,.5)'); hg.addColorStop(.25,'rgba(222,160,70,.16)'); hg.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle=hg; ctx.fillRect(0,hy-H*.2,W,H*.35);
    ...
    // Valley river
    ctx.save(); ctx.translate(px*-2,py*-1);
    ...
    // City lights
    ctx.save(); ctx.translate(px*-2,py*-1);
    for(let i=0;i<95;i++){const x=rand(W*.08,W*.92), y=rand(H*.78,H*.91);ctx.globalAlpha=rand(.18,.65);ctx.fillStyle='#edc46d';ctx.fillRect(x,y,rand(1,2),rand(1,2));} ctx.globalAlpha=1;ctx.restore();

    // Mosque silhouette and minarets
    ctx.save(); ctx.translate(px*-2,py*-1);
    ...
    for(const p of particles){const yy=(p.y-(s*p.speed*30)%H+H)%H;ctx.globalAlpha=.15+.35*(.5+.5*Math.sin(s+p.phase));ctx.fillStyle='#f1ce7e';ctx.beginPath();ctx.arc(p.x+px*-8,yy+py*-5,p.r,0,Math.PI*2);ctx.fill();} ctx.globalAlpha=1;
