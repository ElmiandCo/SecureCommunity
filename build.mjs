...
// Supabase credentials are supplied by Vercel at build time. Production should
// always have them; previews/development may intentionally omit them for
// static/coming-soon builds, so do not make the entire build fail just because
// an environment does not need the authenticated app yet.
const url = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || "";
if (!url || !key) {
  console.warn("Supabase environment variables are not configured for this Vercel build. Authenticated features will remain unavailable until they are supplied.");
}
