...
for(const f of ["index.html","styles.css","input-fix.css","interaction-safe.css","app.js","lesson-fix.js","profile-auth-fix.js","social-features.js","member-discovery.js","notes-post-search.js","islamic-tags-xp.js","notes-social-upgrade.js","profile-private-notes.js","header-home-lock.js","community-posts-notes.js","avatar-profile-picker.js"])if(fs.existsSync(f))fs.copyFileSync(f,path.join("dist",f));
if(fs.existsSync("home-dashboard-safe.js"))fs.copyFileSync("home-dashboard-safe.js","dist/home-dashboard.js");
if(fs.existsSync("interaction-safe.js"))fs.copyFileSync("interaction-safe.js","dist/interaction-safe.js");
...
if(!supabaseSource)throw new Error("Supabase SDK not found in installed package.");
fs.copyFileSync(supabaseSource,"dist/supabase.js");
...
for(const f of ["home-dashboard.js","lesson-fix.js","profile-auth-fix.js","social-features.js","member-discovery.js","notes-post-search.js","islamic-tags-xp.js","notes-social-upgrade.js","profile-private-notes.js","header-home-lock.js","community-posts-notes.js","avatar-profile-picker.js","interaction-safe.js"])if(fs.existsSync(f)&&!html.includes(`src="${f}"`))html=html.replace("</body>",`<script src="${f}"></script>\n</body>`);
...
if(!html.includes('href="interaction-safe.css"')&&fs.existsSync("interaction-safe.css"))html=html.replace("</head>",`<link rel="stylesheet" href="interaction-safe.css">\n</head>`);
fs.writeFileSync("dist/index.html",html);fs.writeFileSync("dist/config.js",`window.APP_CONFIG=${JSON.stringify({SUPABASE_URL:url,SUPABASE_ANON_KEY:key})};\n`);console.log("Built known-good runtime with navigation safety layer.");
