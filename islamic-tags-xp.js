(() => {
  ...
  let tags = [], selected = new Set(), activeTag = '';
  ...
  async function loadTags(){
    const r=await db.from('islamic_tags').select('tag,label,emoji,sort_order').order('sort_order');
    if(r.error) return;
    tags=r.data||[];
  }
  ...
  async function decoratePosts(){
    ...
    const r=await db.from('post_tags').select('post_id,tag').in('post_id',ids);
    if(r.error) return;
    const by={}; (r.data||[]).forEach(x=>(by[x.post_id]??=[]).push(x.tag));
    cards.forEach(card=>{
      card.querySelector('.post-tags')?.remove();
      const row=by[card.dataset.post]||[]; if(!row.length) return;
      ...
      const actions=card.querySelector('.post-actions'); actions?.before(div);
      div.querySelectorAll('[data-filter-tag]').forEach(b=>b.onclick=()=>filterTag(b.dataset.filterTag));
    });
  }
  ...
  function renderGlobalTags(){
    ...
    box.innerHTML=tags.map(t=>`<button type="button" class="islamic-tag" data-global-tag="${esc(t.tag)}">${esc(t.emoji)} ${esc(t.label)}</button>`).join('') + (activeTag?`<button type="button" class="tag-filter-clear" id="clearTagFilter">Clear ×</button>`:'');
    box.querySelectorAll('[data-global-tag]').forEach(b=>b.onclick=()=>filterTag(b.dataset.globalTag));
    $('clearTagFilter')?.addEventListener('click',()=>filterTag(''));
  }
  function filterTag(tag){
    ...
    document.querySelectorAll('[data-post]').forEach(card=>{
      ...
      const has=[...card.querySelectorAll('[data-filter-tag]')].some(x=>x.dataset.filterTag===tag);
      card.style.display=has?'':'none';
    });
  }

  function hookPublish(){
    const btn=$('publish'); if(!btn || btn.dataset.tagsHooked) return;
    btn.dataset.tagsHooked='1';
    const original=btn.onclick;
    btn.onclick=async e=>{await original?.(e); await new Promise(r=>setTimeout(r,250)); await attachTagsToNewestPost();};
  }

  async function init(){
    styles(); await loadTags();
    renderTagChooser(); ensureFilterBar(); hookPublish();
    await decoratePosts();
    const feed=$('feed');
    if(feed && !feed.dataset.tagObserver){
      feed.dataset.tagObserver='1';
      new MutationObserver(()=>{hookPublish();decoratePosts();}).observe(feed,{childList:true,subtree:true});
    }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',init); else init();
  setInterval(()=>{renderTagChooser();hookPublish();},1200);
})();
