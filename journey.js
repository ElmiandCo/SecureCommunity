...
function renderQuestion(){
 ...
 $('player').innerHTML=`<div class="question"><div class="kicker">${q.is_lock_in?'🔒 LOCK IN':'QUESTION'} · ${Number(q.points||0).toLocaleString()} XP</div><p class="question-count">Question ${n} of ${total}</p><h2 class="question-text">${esc(q.question_text)}</h2><div class="answers">${options.map((x,i)=>`<button class="answer ${state.answers[state.index]===String(x)?'selected':''}" data-a="${i}"><span class="answer-letter">${String.fromCharCode(65+i)}</span><span>${esc(x)}</span></button>`).join('')}</div><div class="actions"><button class="btn secondary" id="quit">Exit</button><button class="btn" id="next" ${state.answers[state.index]===null?'disabled':''}>${n===total?'Submit':'Continue'}</button></div></div>`;
