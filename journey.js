...
function renderQuestion(){const q=state.questions[state.index],n=state.index+1,total=state.questions.length;$('player').innerHTML=`<div class="question"><div class="kicker">${q.is_lock_in?'🔒 LOCK IN':'QUESTION'} · ${Number(q.points||0).toLocaleString()} XP</div><p>Question ${n} of ${total}</p><h2>${esc(q.question_text)}</h2><div class="answers">${(q.options||[]).map((x,i)=>`<button class="answer ${state.answers[state.index]===String(i)?'selected':''}" data-a="${i}">${String.fromCharCode(65+i)} · ${esc(x)}</button>`).join('')}</div><div class="actions"><button class="btn secondary" id="quit">Exit</button><button class="btn" id="next" ${state.answers[state.index]===null?'disabled':''}>${n===total?'Submit':'Continue'}</button></div></div>`;document.querySelectorAll('[data-a]').forEach(b=>b.onclick=()=>{state.answers[state.index]=b.dataset.a;renderQuestion()});$('quit').onclick=()=>{$('catalog').style.display='block';$('player').innerHTML='';state=null};$('next').onclick=()=>{if(state.answers[state.index]===null)return;if(state.index<state.questions.length-1){state.index++;renderQuestion()}else finish()}}
