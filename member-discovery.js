(() => {
  ...
  async function render(){
    ...
    const filter=()=>{
      ...
      grid.dataset.omPeopleRendering='1';grid.innerHTML=out.slice(0,8).map(card).join('')||'<div class="people-empty">No members match those filters.</div>';delete grid.dataset.omPeopleRendering;wire();window.OneMuslimProfileFollows?.decorateCards?.();
    };
