/* OneMuslim Unified Profile Builder v6 — single editor, schema-safe save. */
(function(){
  'use strict';
  ...
  function styles(){if(document.getElementById('om-pb-v6-styles'))return;const s=document.createElement('style');s.id='om-pb-v6-styles';s.textContent=`
  .om-pb-tabs{gap:8px}.om-pb-tab{min-width:62px;min-height:62px;font-size:24px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:3px}.om-pb-tab small{font-size:11px}.om-pb-profile-fields{display:grid;gap:14px}.om-pb-profile-fields label{display:grid;gap:6px}.om-pb-field-row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.om-pb-profile-fields input,.om-pb-profile-fields textarea,.om-pb-profile-fields select{width:100%;box-sizing:border-box}.om-pb-current-avatar{display:flex;align-items:center;gap:12px;margin-bottom:14px}.om-pb-avatar-photo{width:88px;height:88px;border-radius:50%;overflow:hidden;border:3px solid #fff;box-shadow:0 8px 25px #0002;background:#edf4ef;display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:800}.om-pb-avatar-photo img{width:100%;height:100%;object-fit:contain}.om-pb-legacy-note{padding:13px 15px;border:1px solid #cbded4;border-radius:14px;background:#f3f7f4;color:#1f5b49}.om-pb-field-xp{font-size:12px;color:#2d705b;font-weight:700}@media(max-width:620px){.om-pb-field-row{grid-template-columns:1fr}}
  ...
  function avatarSrc(p){
    ...
    if(p.avatar_config&&typeof p.avatar_config==='object'&&(p.avatar_config.asset||p.avatar_config.url))return p.avatar_config.asset||p.avatar_config.url;
    return '';
  }
  ...
  function open(){
    ...
    o.innerHTML=`<section class="om-pb-shell">
      ...
      <section class="om-pb-panel" data-panel="background"><div class="om-pb-heading"><span class="om-pb-kicker">ATMOSPHERE</span><h3>Set your background</h3><p>Choose the atmosphere behind your profile.</p></div><div class="om-pb-grid">${backgrounds.map((b,i)=>`<button type="button" class="om-pb-card om-pb-bg ${p.profile_background===(i?'': 'default')?'':''}" data-bg="${esc(i?'': 'default')}" style="background:${b[1]}"><strong>${b[0]}</strong></button>`).join('')}</div></section>
      <section class="om-pb-panel" data-panel="style"><div class="om-pb-heading"><span class="om-pb-kicker">PERSONALIZE</span><h3>Make it yours</h3><p>Premium avatar styling unlocks at 10,000 XP.</p></div><div class="om-pb-card"><label>Avatar finish</label><div class="om-pb-choices"><button type="button" class="om-pb-choice pb-package ${p.avatar_package!=='platinum_package'?'selected':''}" data-package="default"><span class="fake-avatar">◉</span>Original</button><button type="button" class="om-pb-choice pb-package ${p.avatar_package==='platinum_package'?'selected':''}" data-package="platinum_package" ${unlocked?'':'disabled'}>Platinum${unlocked?'':' 🔒'}</button></div><div class="om-pb-note">${unlocked?'Platinum unlocked.':'Platinum unlocks at 10,000 XP. '+Math.max(0,10000-xp()).toLocaleString()+' XP remaining.'}</div></div><div class="om-pb-card" style="margin-top:14px"><label>Accent color</label><div class="om-pb-swatches">${accents.map(a=>`<button type="button" class="om-pb-swatch ${p.profile_accent===a[0]?'selected':''}" data-accent="${a[0]}" style="background:${a[1]}" aria-label="${a[0]}"></button>`).join('')}</div></div></section>
    ...
    o.querySelectorAll('.pb-package').forEach(b=>b.onclick=()=>{if(b.disabled)return;draft.avatar_package=b.dataset.package;if(draft.avatar_package==='platinum_package')draft.avatar_url=`assets/avatars/${draft.avatar_gender==='female'?'platinum-female.PNG':'platinum-male.PNG'}`;preview()});
    ...
    o.querySelector('#pbSave').onclick=async()=>{
      ...
      try{
        ...
        const payload={display_name:o.querySelector('#pfDisplay').value.trim(),username,first_name:o.querySelector('#pfFirst').value.trim(),last_name:o.querySelector('#pfLast').value.trim(),bio:o.querySelector('#pfBio').value.trim(),location:o.querySelector('#pfLocation').value.trim(),city:o.querySelector('#pfCity').value.trim(),state:o.querySelector('#pfState').value.trim(),country:o.querySelector('#pfCountry').value.trim(),gender:o.querySelector('#pfGender').value,website:o.querySelector('#pfWebsite').value.trim(),group_team:o.querySelector('#pbGroup').value,profile_visibility:o.querySelector('#pbPrivacy').value,avatar_url:selectedAvatar||null,avatar_gender:draft.avatar_gender,avatar_package:draft.avatar_package,profile_background:draft.profile_background||'default',profile_accent:draft.profile_accent||'emerald'};
        if(draft.avatar_config&&typeof draft.avatar_config==='object')payload.avatar_config={...draft.avatar_config,asset:selectedAvatar||draft.avatar_config.asset||null,gender:draft.avatar_gender,package:draft.avatar_package};
        ...
        toast('Profile saved successfully.');close();
