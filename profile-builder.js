/* OneMuslim Unified Profile Builder v7 — AvatarStudio identity, finish separation, and schema-safe save. */
(function(){
  'use strict';
  ...
  function styles(){if(document.getElementById('om-pb-v7-styles'))return;const s=document.createElement('style');s.id='om-pb-v7-styles';s.textContent=`
  ...
  function open(){
    ...
    o.querySelectorAll('.pb-gender').forEach(b=>b.onclick=()=>{draft.avatar_gender=b.dataset.gender;draft.avatar_package='default';draft.avatar_url=REGULAR[draft.avatar_gender];draft.avatar_config=null;preview()});
    o.querySelectorAll('.pb-package').forEach(b=>b.onclick=()=>{if(b.disabled)return;draft.avatar_package=b.dataset.package;draft.avatar_url=avatarSrc(draft);preview()});
    ...
    o.querySelector('#pbSave').onclick=async()=>{
      ...
      try{
        ...
        const payload={display_name:o.querySelector('#pfDisplay').value.trim(),username,first_name:o.querySelector('#pfFirst').value.trim(),last_name:o.querySelector('#pfLast').value.trim(),bio:o.querySelector('#pfBio').value.trim(),location:o.querySelector('#pfLocation').value.trim(),city:o.querySelector('#pfCity').value.trim(),state:o.querySelector('#pfState').value.trim(),country:o.querySelector('#pfCountry').value.trim(),gender:o.querySelector('#pfGender').value,website:o.querySelector('#pfWebsite').value.trim(),group_team:o.querySelector('#pbGroup').value,profile_visibility:o.querySelector('#pbPrivacy').value,avatar_url:selectedAvatar,avatar_gender:draft.avatar_gender,avatar_package:draft.avatar_package,avatar_config:null,profile_background:draft.profile_background||'default',profile_accent:draft.profile_accent||'emerald'};
