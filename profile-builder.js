...
(function(){
  ...
  function open(){
    ...
    o.querySelectorAll('.pb-gender').forEach(b=>b.onclick=()=>{draft.avatar_gender=b.dataset.gender;draft.avatar_package='default';draft.avatar_url=REGULAR[draft.avatar_gender];draft.avatar_config={gender:draft.avatar_gender,package:draft.avatar_package,background:draft.profile_background,accent:draft.profile_accent};preview()});
    o.querySelectorAll('.pb-package').forEach(b=>b.onclick=()=>{if(b.disabled)return;draft.avatar_package=b.dataset.package;draft.avatar_url=avatarSrc(draft);draft.avatar_config={gender:draft.avatar_gender,package:draft.avatar_package,background:draft.profile_background,accent:draft.profile_accent};preview()});
    o.querySelectorAll('.om-pb-bg').forEach((b,i)=>b.onclick=()=>{draft.profile_background=i?backgrounds[i][0]:'default';draft.avatar_config={gender:draft.avatar_gender,package:draft.avatar_package,background:draft.profile_background,accent:draft.profile_accent};o.querySelectorAll('.om-pb-bg').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');preview()});
    o.querySelectorAll('[data-accent]').forEach(b=>b.onclick=()=>{draft.profile_accent=b.dataset.accent;draft.avatar_config={gender:draft.avatar_gender,package:draft.avatar_package,background:draft.profile_background,accent:draft.profile_accent};o.querySelectorAll('[data-accent]').forEach(x=>x.classList.toggle('selected',x===b))});
    ...
    o.querySelector('#pbSave').onclick=async()=>{
      ...
      try{
        ...
        const payload={display_name:o.querySelector('#pfDisplay').value.trim(),username,first_name:o.querySelector('#pfFirst').value.trim(),last_name:o.querySelector('#pfLast').value.trim(),bio:o.querySelector('#pfBio').value.trim(),location:o.querySelector('#pfLocation').value.trim(),city:o.querySelector('#pfCity').value.trim(),state:o.querySelector('#pfState').value.trim(),country:o.querySelector('#pfCountry').value.trim(),gender:o.querySelector('#pfGender').value,website:o.querySelector('#pfWebsite').value.trim(),group_team:o.querySelector('#pbGroup').value,profile_visibility:o.querySelector('#pbPrivacy').value,avatar_url:selectedAvatar,avatar_gender:draft.avatar_gender,avatar_package:draft.avatar_package,avatar_config:{...(p.avatar_config&&typeof p.avatar_config==='object'?p.avatar_config:{}),gender:draft.avatar_gender,package:draft.avatar_package,background:draft.profile_background||'default',accent:draft.profile_accent||'emerald'},avatar_updated_at:new Date().toISOString(),profile_background:draft.profile_background||'default',profile_accent:draft.profile_accent||'emerald'};
        const data=await safeUpdate(sb,user.id,payload);
        window.profile={...p,...data,...payload,avatar_config:payload.avatar_config,profile_background:payload.profile_background,profile_accent:payload.profile_accent};
