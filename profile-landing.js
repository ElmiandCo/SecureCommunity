...
async function syncProfile(){if(syncing)return window.profile||null;const sb=client();if(!sb)return null;syncing=true;try{const {data:{user}}=await sb.auth.getUser();if(!user)return null;const {data,error}=await sb.from('profiles').select('*').eq('id',user.id).maybeSingle();if(error)throw error;if(data){const normalized=window.OneMuslimProfileSystem?.normalize?.(data)||data;window.profile=normalized;return normalized}return window.profile||null}catch(e){console.error('Profile sync failed:',e);return window.profile||null}finally{syncing=false}}
